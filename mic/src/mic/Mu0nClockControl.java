/*
 March 26th 2021
 Addind some very needed features to the clock:

 -sync state of the clock to incoming/returning players/observers
 -ability to reset the clock
 -ability to set the clock at a specific time
 -verbose warnings for starting, resetting, setting, pausing and resuming

 This is the parent class which can own several Mu0nClocks (just the one is typically used though)
 */
 ...
 */
public class Mu0nClockControl extends AbstractConfigurable
        implements CommandEncoder, GameComponent, UniqueIdManager.Identifyable {
    protected static final UniqueIdManager ID_MGR = new UniqueIdManager("Mu0nClockControl"); //$NON-NLS-1$

    protected List<Mu0nClock> mu0nClocks = new ArrayList<>();  // List of individual chess clocks attached to our control
    ...
    public Mu0nClockControl() {
        setConfigureName(Resources.getString("Editor.Mu0nClock.chess_clock_control"));
     ...
     */
    public List<Mu0nClock> getChessClocks() {
        return mu0nClocks;
    }
    ...
    public void addTo(Buildable parent) {
        ...
        for (final ChessClock c : mu0nClocks) {
            c.addToToolbar();
        }
    ...
    public String[] getAttributeDescriptions() {
        return new String[] {
                ...
                Resources.getString("Editor.Mu0nClock.show_clocks_hotkey"),
                Resources.getString("Editor.Mu0nClock.start_next_clock_hotkey"),
                Resources.getString("Editor.Mu0nClock.pause_all_clocks_hotkey"),
                Resources.getString("Editor.Mu0nClock.show_tenths_of_seconds"),
                Resources.getString("Editor.Mu0nClock.show_seconds"),
                Resources.getString("Editor.Mu0nClock.show_hours"),
                Resources.getString("Editor.Mu0nClock.show_days"),
                Resources.getString("Editor.Mu0nClock.allow_reset")
        };
    }
     ...
     */
    public static Mu0nClockControl getInstance() {
        return GameModule.getGameModule().getAllDescendantComponentsOf(Mu0nClockControl.class).stream().findFirst().orElse(null);
    }
    ...
    public int getClocksTicking() {
        ...
        if (instanceIsActive && (getInstance() != null)) {
            for (final Mu0nClock c : getInstance().chessclocks) {
                if (c.isTicking()) {
                    running++;
                }
            }
    ...
    public Command startNextClock() {
        ...
        if (mu0nClocks.isEmpty()) { // If we don't have any clocks, can't start one.
            return command;
        }

        ChessClock first = null;
        boolean found = false;
        for (final ChessClock clock : mu0nClocks) {
            if (first == null) {
                first = clock; // Mark first clock, in case we run off the end and that's the one we need
                // to start.
                if (mu0nClocks.size() == 1) {
    ...
    public void updateAllClocks() {
        if (instanceIsActive && (getInstance() != null)) {
            for (final Mu0nClock c : getInstance().mu0nClocks) {
                c.setTimerButton();
            }
        }
    }
     ...
     */
    public void addChessClock(Mu0nClock clock) {
        mu0nClocks.add(clock);
    }
     ...
     */
    public void removeChessClock(Mu0nClock clock) {
        mu0nClocks.remove(clock);
    }
    ...
    public static class IconConfig implements ConfigurerFactory {
        ...
        public Configurer getConfigurer(AutoConfigurable c, String key, String name) {
            return new IconConfigurer(key, name, ((Mu0nClockControl) c).chessClockButton.getAttributeValueString(ICON));
        }
    }
    ...
    public static class TimeStyleConfig extends TranslatableStringEnum {
        ...
        public String[] getI18nKeys(AutoConfigurable target) {
            return new String[] {
                    "Editor.Mu0nClock.always",
                    "Editor.Mu0nClock.auto",
                    "Editor.Mu0nClock.never" };
        }
    }
